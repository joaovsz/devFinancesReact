...
- [x] Migrar formularios principais para `react-hook-form` + `zod`.
- [x] Isolar/remover dependencias de Supabase/Auth da experiencia principal local-first.
...
- [x] Revisar outros componentes/paginas para remover qualquer duplicacao remanescente de regra financeira.
...
- [x] Atualizar/remover `useMemo`, `useCallback` e memoizacoes manuais apenas depois dos testes passarem.

## Formularios

- [x] Declarar `react-hook-form` e `zod` no `package.json`.
- [x] Criar pasta de schemas de formularios.
- [x] Migrar `TransactionForm.tsx`.
- [x] Migrar formularios de cartao em `Cards.tsx`.
- [x] Migrar formularios de planejamento em `PlanningPage.tsx`.
- [x] Manter chips rapidos, numpad virtual, sliders e selects point-and-click.
...
- [x] Marcar fatura como paga nao muda Saldo Total nem Saidas.
- [x] Fatura paga libera limite, mas nao altera a linha do tempo exibida das parcelas.
- [x] Transacoes mostra somente itens do Mes Operacional ativo.
...
- [x] Garantir `tsconfig.json` com `strict: true` e sem afrouxamentos novos.
...
- [x] Nenhum arquivo novo reintroduz regra duplicada de cartao/fatura fora do modulo de dominio.

### Fase 2: Tipagem e Schemas

Objetivo: criar uma fronteira unica de validacao para entrada de dados.

- [x] Criar `src/types/forms.ts` se necessario para tipos de input/output dos formularios.
...
- [x] Exportar schemas por um barrel simples apenas se isso reduzir imports repetidos sem esconder responsabilidade.

Critério de saida da fase:

- [x] Toda gravacao de transacao e cartao passa por `zod`.
- [x] Conversoes de moeda, parcela, datas e dias de cobranca ficam normalizadas em um ponto unico.

### Fase 3: Migracao de Formularios

Objetivo: reduzir estado manual espalhado, melhorar validacao e manter UX point-and-click.

- [x] Migrar `src/components/transactions/TransactionForm.tsx` para `react-hook-form` + `zod`.
- [x] Migrar formularios internos de `src/components/Cards.tsx`.
- [x] Avaliar quebra de responsabilidade em `Cards.tsx`; se necessario, extrair formularios para `src/components/cards/`.
- [x] Migrar entradas de `src/pages/PlanningPage.tsx`.
- [x] Preservar chips rapidos, sliders e numpad virtual como adaptadores controlados do `react-hook-form`.
- [x] Garantir mensagens de erro curtas, calmas e acionaveis.

Critério de saida da fase:

- [x] Nenhum formulario principal depende de cascata manual de `useState` para cada campo.
- [x] Validacoes de valor, data, recorrencia, parcela e cartao acontecem antes de persistir no store.

### Fase 4: Isolamento Local-First

Objetivo: tornar a experiencia principal independente de autenticacao e sincronizacao remota.

- [x] Mapear todos os pontos de acoplamento com Supabase:
- [x] `src/context/AuthContext.tsx`
- [x] `src/components/auth/LoginPage.tsx`
- [x] `src/hooks/useSupabaseSync.ts`
- [x] `src/services/supabase-sync.ts`
- [x] `src/lib/supabase.ts`
- [x] `src/pages/SettingsPage.tsx`
- [x] Definir se o modo remoto sera removido do fluxo principal ou escondido atras de feature flag local.
- [x] Garantir boot da aplicacao sem dependencia de sessao, token ou variavel externa.
- [x] Preservar import/export JSON como estrategia oficial de portabilidade.

Critério de saida da fase:

- [x] Usuario consegue usar o app do zero sem login.
- [x] Nenhuma tela principal quebra quando Supabase nao estiver configurado.
- [x] Fluxo de configuracao remota, se mantido, passa a ser opcional e desacoplado.
...
- [x] compra parcelada atravessando meses
- [x] cartao com fechamento diferente do vencimento
- [x] ajuste manual positivo e negativo
- [x] pagamento de fatura
- [x] Escrever testes para todos os cenarios listados em "Testes Obrigatorios".
- [x] Adicionar testes para fallback de `chargeDay` ausente.
- [x] Adicionar testes de nao-regressao para liberacao de limite versus fluxo de caixa.

Critério de saida da fase:

- [x] A suite de dominio cobre as regras que hoje sustentam `Cards`, `Transactions` e projecoes.
- [x] Toda correcao futura de cartao/fatura passa primeiro por teste.

### Fase 6: Limpeza e Otimizacao React

Objetivo: simplificar o codigo depois que a seguranca funcional estiver estabelecida.

- [x] Revisar `useMemo` e `useCallback` em `src/components/Cards.tsx`.
- [x] Revisar `useMemo` e derivacoes em `src/components/Transactions.tsx`.
- [x] Remover memoizacao manual que o React Compiler possa absorver.
- [x] Extrair blocos grandes com responsabilidade misturada em componentes menores quando isso reduzir risco de regressao.
- [x] Evitar refactors cosmeticos sem ganho claro de manutencao.

Critério de saida da fase:

- [x] Codigo mais curto e previsivel sem alterar regra de negocio.
- [x] Nenhuma otimizacao e feita antes da cobertura minima do dominio.
...
- [x] Limite reduz no momento da compra.
- [x] Fluxo de caixa so sente a despesa no vencimento da fatura.
- [x] Nome operacional da fatura continua coerente com o ciclo exibido ao usuario.
- [x] Ajuste manual edita a diferenca, nao duplica despesa.

### Parcelamentos

- [x] Parcela aparece com progresso cronologico correto.
- [x] Parcelas antigas continuam funcionando mesmo sem todos os metadados novos.
- [x] Ultima parcela desaparece do mes seguinte sem lixo residual no resumo.

### Formularios

- [x] Campos obrigatorios e formatos invalidos sao barrados antes de salvar.
- [x] Fluxos point-and-click continuam mais rapidos que digitacao pura.
- [x] Erros de validacao nao quebram a experiencia mobile.

### Local-First

- [x] App sobe sem backend.
- [ ] Persistencia local continua integra apos reload.
- [x] Importacao/exportacao JSON cobre o estado relevante do usuario.
...
- [x] Dominio financeiro estiver coberto por testes automatizados.
- [x] Formularios principais estiverem em `react-hook-form` + `zod`.
- [x] Experiencia principal estiver funcional sem Auth/Supabase.
- [x] Build e testes passarem localmente.
